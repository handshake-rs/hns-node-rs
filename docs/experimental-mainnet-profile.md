...
`hsrd` uses **Shakescape Experimental V1 — Not an official Handshake protocol
...
- Registry: `Shakescape Experimental Handshake P2P Registry`
- Registry version: `1`
...
These values are stable only within Shakescape Experimental Registry v1. They are
not official global Handshake assignments.

## Connection behavior

The node advertises the ordinary network service and the Shakescape extension
...
being implied by Shakescape negotiation.

Ordinary VERSION/VERACK completes first. An outbound peer then initiates the
first `SHAKESCAPE_EXT` exchange only if the remote VERSION advertised the extension
...
cancelled; any Ack observed after the deadline is rejected and Shakescape remains
...
consumer. Unknown Shakescape subprotocols are bounded and rejected without being
